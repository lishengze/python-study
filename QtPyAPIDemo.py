...
# 分时行情回调函数示例
_ofilename_sts = 'DemoQuoteOut_Sts.txt'
def OnSubSts(data):
    global g_StsLeft
    g_StsLeft -= 1
    dictDataStr = ''
    ...
    with open(_ofilename_sts, 'a') as of:
        of.writelines("Sts: " + str(localTime) + ":" + dictDataStr + "\n")

    return;

# L2Quote行情回调函数示例
_ofilename_l2 = 'DemoQuoteOut_L2.txt'
def OnSubL2Quote(data):
    ...
    #print str(localTime), dictDataStr
    with open(_ofilename_l2, 'a') as of:
        of.writelines("L2Quote: " + str(localTime) + ":" + dictDataStr + "\n")

    return;

# L1Quote行情回调函数示例
_ofilename_l1 = 'DemoQuoteOut_L1.txt'
def OnSubQuote( data ):
    ...
    #print str(localTime), dictDataStr
    with open(_ofilename_l1, 'a') as of:
...
def SubDemo():
    ...
    # L1 Quote订阅、接收及退订示例
    if 1:
        #os.remove(_ofilename_l1)

        # 注册L1行情回调函数，推荐在发起订阅之前注册相关行情回调方法
        RegSubQuoteCB(OnSubQuote)
    ...
    # L2 Quote订阅、接收及退订示例
    if 1:
        #os.remove(_ofilename_l2)

        # 注册L2行情回调函数，推荐在发起订阅之前注册相关行情回调方法
        RegSubL2QuoteCB(OnSubL2Quote)
    ...
    # 分时行情订阅、接收及退订示例
    if 1:
        #os.remove(_ofilename_sts)

        # 注册分时行情回调函数，推荐在发起订阅之前注册相关行情回调方法
        RegSubStsCB(OnSubSts)
...
if __name__=='__main__':
    ...
    if 1:
        ...
        if ret == 0:
            ...
            if g_toFile:
                ...
                ret = dataCols.to_csv('Test_GetExchanges.csv')
        else:
    ...
    if 1:
        markets = ["SSE"]
        fields = ["CalendarDate", "Market", "IsOpen", "TradingType", "IsNightTrading" ]
    ...
    if 1:
        ...
        if ret == 0:
            ...
            if g_toFile:
                dataCols = ConvertStr(dataCols)
                dataCols.to_csv('Test_GetPlateSymbols.csv')
        else:
    ...
    if 1:
        ...
        timePeriods = [['2017-02-01 00:00:00.000', '2017-03-01 00:00:00.000'], \
                       ['2017-04-01 00:00:00.000', '2017-05-01 00:00:00.000']]
        ...
        if ret == 0:
            ...
            if g_toFile:
                dataCols = ConvertStr(dataCols)
                dataCols.to_csv('Test_GetDataByTime.csv')
        else:
