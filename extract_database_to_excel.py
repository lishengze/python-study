...
def get_cleared_secode_list(oriSecodeList, databaseObj):
...
def main():
    ...
    # host = "192.168.211.162"
    host = "localhost"
    fileName = "D:\excel\沪深300成分股_Code.xlsx"
    key_list = ["TDATE", "TIME"]
    indexcode = "SH000300"
    start_date = "20150130"
    end_date = "20180417"
    ...
    databaseObj = MarketDatabase(db=datatype, host=host)
    execlObj = EXCEL()
    oriSecodeList = get_secode_list_from_excel(fileName, 'ori')
    # print_list('excel_list: ', oriSecodeList)

    # oriSecodeList.append(indexcode)
    # oriSecodeList = oriSecodeList[0:40]
    cleared_secode_list = get_cleared_secode_list(oriSecodeList, databaseObj)
    ori_data = databaseObj.get_histdata_bytime(start_date, end_date, indexcode, value_list=key_list)
    index_time_list = get_time_list(ori_data)
    index_str_time_list = trans_time_list(index_time_list)
    index_str_time_list.insert(0, "time\\code")

    b_single_write = True

    # oriSecodeList = ["SH600000", "SH600008", "SH000300"]
    if b_single_write:
        readdata_starttime = datetime.datetime.now()
        # key_value_list = ["turnover"]
        key_value_list = ["tclose"]
        # key_value_list = ["turnover", "votrunover"]
        ...
        for key_value in key_value_list:
            ...
            des_file_name = 'D:\excel\\test_python_%s_%s_%d.xlsx' % \
                            (datatype, key_value, len(oriSecodeList))
            all_kevalue_list = []
            all_kevalue_list.append(index_str_time_list)

            print ("------- Start Read " + sheet_name +" Data -------")
            complete_count = 0
            for secode in oriSecodeList:
                curr_kevalue_list = []
                ...
                curr_kevalue_list.insert(0, secode)
                all_kevalue_list.append(curr_kevalue_list)

                # print ("secode: ", secode, ", key numb: ", len(curr_kevalue_list), \
                #     ", delete numb: ", len(delete_data), ", delist numb: ", len(delist_data), \
                #     ", unlist numb: " , len(missed_time), ", index: ", index)

                complete_count += 1
                print ('complete_count %d' % (complete_count))
            ...
            print ("\n----- Start Write data To ", sheet_name, " -----")

            trans_data = trans_matrix(all_kevalue_list)
            execlObj.write_all_data(trans_data, des_file_name, sheet_name)
