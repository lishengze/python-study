	...
	for line in result_info.split('\n'):
		...
		if len(line_list) >= 4:
			...
			if status == cfg.CMD_UNDEPLOY_TIP:
				...
				cmdline = cfg.FLAG_BLANK.join(status_list[2:])
			...
			while 1:
				...
				if buf:
					...
					#通知任务状态变化（包括新建计划任务）
						...
						if cfg.FLAG_DEBUG:
							print('get {%s} from getTaskInfo'%(info))
						...
						if task_info.state == cfg.FLAG_TASK_READY:
							...
							if cfg.FLAG_DEBUG:
								print('get {%s} from getTaskResult'%(result_info))
					...
					###!!!文件上传
						...
						filepath = cfg.WORK_PATH_TEMP + cfg.SEP_COMM + filename + cfg.DOT + cfg.PID

						file_md5_new = md5sum_buf(buf)
						print("filemd5_new: " + file_md5_new)
						print("filemd5: " + file_md5)
						if file_md5 == file_md5_new:
							f = open(filepath, 'wb+')
							f.write(buf)
							f.close()

							shutil.copyfile(filepath, FileDst)
							print(filepath, FileDst)
