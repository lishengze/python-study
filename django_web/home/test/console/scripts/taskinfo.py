...
def recv_end(the_socket):
	...
	while True:
		...
		if len(data) < len(cfg.TIP_INFO_EOF) and len(total_data) >0:
			#check if end_of_data was split
			data = total_data.pop() + data
		if data.endswith(cfg.TIP_INFO_EOF):
...
def getFRspHead(buf):
	return getInfo(buf, cfg.TIP_HEAD_FRSP, cfg.NEWLINE)

def getFRspInfo(head_info):
	seq_id = head_info[head_info.find(cfg.CLOSE_BRACKET)+len(cfg.CLOSE_BRACKET):head_info.find(cfg.DCOLON)]
	filename = head_info[head_info.find(cfg.DCOLON)+len(cfg.DCOLON):head_info.rfind(cfg.DCOLON)]
...
def genFRspHead(req_id, filename, status):
	return cfg.TIP_HEAD + cfg.TIP_HEAD_FRSP + str(req_id) + cfg.DCOLON + filename + cfg.DCOLON + str(status) + cfg.NEWLINE
...
class TaskInfo:
	...
	def shouldExec(self, timestamp):
		if timestamp >= self.exec_time and (not self.isTimeOut(timestamp)):
			return True
		else:
			return False
